...
func TestRunQueueWorkersRetriesFromPersistedFailedState(t *testing.T) {
	...
	cfg.SchedulerBatchSize = 10
	cfg.CheckLeaseTimeout = time.Minute
	...
	go RunQueueScheduler(ctx, service, queue, cfg, logger)
...
func TestHandleQueueDeliveryDoesNotRetryBeforeLeaseStateChanges(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "persist retryable check job failure") {
		t.Fatalf("error = %q, want persisted retry context", err.Error())
	}
	if repo.addCheckAttempts() == 0 {
		t.Fatal("AddCheck was not attempted")
	}
	if repo.markFailedAttempts() == 0 {
		t.Fatal("MarkJobFailed was not attempted")
	}
...
func TestHandleQueueDeliveryRequeuesInfrastructureFailureWithoutConsumingAttempt(t *testing.T) {
	...
	delivery := QueueDelivery{
		Job: CheckJobMessage{
			JobID:     "job_infrastructure_failure",
			MonitorID: "mon_1",
			Attempt:   2,
		},
		Retryable: true,
...
func (r *failingLeaseTransitionRepository) MarkJobFailed(context.Context, ProcessingLease, string, time.Time, time.Time) error {
	...
	r.markFailedCount++
	r.mu.Unlock()
	return errors.New("transient job state failure")
}
...
func (r *failingLeaseTransitionRepository) markFailedAttempts() int {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.markFailedCount
}
